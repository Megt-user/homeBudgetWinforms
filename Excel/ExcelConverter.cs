        ...
        public JArray GetJsonArrayfromExcelfile(string path)
        {
            ...
            if (fi1.Exists)
            {
                using (var cashflowExcelPkg = new ExcelPackage(fi1))
                {
                    ...
                    Log.Information("there is {TransactionCounts} in {ExcelFileName}", jsonArray?.Count, Path.GetFileName(path));
                }
            }
            else
            {
                Log.Error("Can't Open File '{ExcelFileName}'",Path.GetFileName(path));
            }
